...
3. **反向查询接口**：通过农历日期、节日名称、节气名称反向查询对应的公历日期
4. **条件查询接口**：在指定日期范围内查询符合条件的日期集合（休息日、工作日、节日、节气）
...
| 反向查询接口     | 农历日期反向查询 | 输入农历日期（如：农历2025年正月初一），返回对应的所有公历日期及详细信息                              |
| 反向查询接口     | 节日名称反向查询 | 输入节日名称（如：春节、中秋节），返回该节日对应的所有公历日期及详细信息                              |
| 反向查询接口     | 节气名称反向查询 | 输入24节气名称（如：立春、冬至），返回该节气对应的所有公历日期及详细信息                              |
| 反向查询接口     | 年份范围查询 | 支持指定年份查询，未指定年份时查询前后5年（共11年）的所有匹配日期                                      |
| 条件查询接口     | 休息日查询 | 在指定日期范围内查询所有休息日（周末和节假日，排除调休工作日）                                          |
| 条件查询接口     | 工作日查询 | 在指定日期范围内查询所有工作日（周一至周五，包含调休工作日）                                            |
| 条件查询接口     | 节日日期查询 | 在指定日期范围内查询所有包含节日的日期                                                                  |
| 条件查询接口     | 节气日期查询 | 在指定日期范围内查询所有包含节气的日期                                                                  |
...
1. 接收MCP协议的反向查询请求
2. 解析查询类型（农历/节日/节气）和查询内容
3. 确定查询年份范围（指定年份或前后5年）
4. 执行反向匹配算法，查找所有符合条件的日期
5. 对每个匹配日期获取完整属性信息
6. 通过MCP协议返回结果数组
...
| 反向查询     | dates     | 包含所有匹配日期的对象数组，每个日期包含完整属性 |
| 条件查询     | dates     | 包含所有符合条件日期的对象数组，每个日期包含完整属性 |
