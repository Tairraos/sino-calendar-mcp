...
export class ReverseQueryEngine {
     ...
     * @returns 匹配的公历日期信息数组
     */
    static queryLunarDate(lunarDate, yearRange) {
        const results = [];
        // 解析农历日期字符串
        ...
        for (const year of yearRange) {
            const solarDates = this.findSolarDatesForLunar(parsedLunar.year, parsedLunar.month, parsedLunar.day, year, parsedLunar.isLeap);
            // 获取每个匹配日期的详细信息
     ...
     * @returns 匹配的公历日期信息数组
     */
    static queryFestival(festivalName, yearRange) {
        const results = [];
        // 查找匹配的 festivals
        for (const year of yearRange) {
            // 获取该年份的所有节日
     ...
     * @returns 匹配的公历日期信息数组
     */
    static querySolarTerm(termName, yearRange) {
        const results = [];
        // 获取节气索引映射
     ...
     * @returns 匹配的日期信息数组
     */
    static queryByDateRange(startDate, endDate, type) {
        const results = [];
     ...
     * @returns 匹配的公历日期数组
     */
    static findSolarDatesForLunar(lunarYear, lunarMonth, lunarDay, targetYear, isLeap) {
        const results = [];
