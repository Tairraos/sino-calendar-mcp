...
export class ErrorHandler {
   ...
   * @returns 执行结果或错误响应
   */
  static async safeExecute<T>(fn: () => T | Promise<T>) {
    try {
   ...
   */
  static validateAndThrow(validation: { isValid: boolean; error?: string }) {
   ...
   * @param result 操作结果
   */
  static logOperation(operation: string, params: any, result?: any) {
    const logEntry = {
      timestamp: new Date().toISOString(),
      operation,
      params,
   ...
   * @param params 操作参数
   */
  static checkSystemLimits(operation: string, params: any) {
    switch (operation) {
