...
export class DateInfoEngine {
   ...
   * @returns 周围日期的重要信息
   */
  static getSurroundingInfo(date: Date, days: number = 7) {
    ...
    // 筛选出有特殊信息的日期
    const importantDates = rangeInfo.dates.filter(
      dateInfo => dateInfo.festival || dateInfo.solarTerm || dateInfo.adjusted,
    );
