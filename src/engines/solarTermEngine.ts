...
export class SolarTermEngine {
  ...
  static getYearSolarTerms(year: number): Map<string, Date> {
    const solarTermsMap = new Map<string, Date>();
   ...
   * @returns 下一个节气的名称和日期
   */
  static getNextSolarTerm(date: Date): { name: string; date: Date } | undefined {
   ...
   * @returns 上一个节气的名称和日期
   */
  static getPreviousSolarTerm(date: Date): { name: string; date: Date } | undefined {
    const year = date.getFullYear();
    const yearSolarTerms = this.getYearSolarTerms(year);

    // 将节气按日期排序
    const sortedSolarTerms = Array.from(yearSolarTerms.entries()).sort(
    ...
    const sortedPrevYearSolarTerms = Array.from(prevYearSolarTerms.entries()).sort(
      (a, b) => a[1].getTime() - b[1].getTime(),
    );
   ...
   * @returns 节气信息
   */
  static getSolarTermByName(name: string) {
