...
export class FestivalEngine {
   ...
   * 获取公历节日
   */
  private static getSolarFestival(date: Date): string | undefined {
   ...
   * 获取农历节日
   */
  private static getLunarFestival(date: Date): string | undefined {
   ...
   * 获取西方节日
   */
  private static getWesternFestival(date: Date): string | undefined {
    const month = String(date.getMonth() + 1).padStart(2, '0');
    const day = String(date.getDate()).padStart(2, '0');
    const dateStr = `${month}-${day}`;

    // 检查固定日期的西方节日
    const fixedFestival = festivals.find(
      f => f.type === 'western' && f.isFixed && f.date === dateStr,
    );
   ...
   * 获取非固定日期的西方节日
   */
  private static getVariableWesternFestival(date: Date): string | undefined {
   ...
   * @returns 复活节的月份和日期
   */
  private static calculateEaster(year: number): { month: number; day: number } {
    // 使用匿名格里高利历算法计算复活节
    const a = year % 19;
   ...
   * 判断是否为除夕
   */
  private static isChineseNewYearEve(date: Date): boolean {
    const solar = Solar.fromDate(date);
    const lunar = solar.getLunar();
